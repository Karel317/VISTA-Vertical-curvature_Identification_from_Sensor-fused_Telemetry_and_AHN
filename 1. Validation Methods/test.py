...
RESULTS_DIR = r"G:\Validation_results\2026_05_22\10_50_00"
...
# ── Alignment settings ───────────────────────────────────────────────────────
# Reference curve whose elevation-start defines the target location.
# Every other profile is shifted so its own elevation-start lands here.
ALIGN_TO_REFERENCE   = True
REFERENCE_SUBSTR     = "Z_positional_tracking"
...
# ── Trim settings ────────────────────────────────────────────────────────────
# Every profile is trimmed to s <= the aligned s of the physical-measurement
# peak elevation. The cutoff is detected automatically, never hardcoded.
TRIM_AT_PHYS_PEAK = True
PHYS_MEAS_SUBSTR  = "Physical_meas"
# Trim the flat approach: left cutoff = aligned slope-start (the alignment ref).
TRIM_AT_SLOPE_START = True
...
def _smooth(z, window_samples):
    """Edge-padded moving average; window forced odd, >=1."""
...
def detect_elevation_start(
    s, z,
    slope_threshold=0.02,    # m per m: minimum sustained rise rate
    baseline_tol=0.03,       # m: how far above baseline counts as "risen"
    smooth_window_m=1.0,     # m: smoothing window for z
    min_rise_len_m=1.0,      # m: rise must be sustained at least this long
    baseline_frac=0.20,      # fraction of lead-in used to estimate baseline
    baseline_pct=5.0,        # percentile within lead-in -> baseline level
    grid_step_m=None,        # uniform resample step; None -> median spacing
):
    """
    Detect the first location where elevation begins a sustained rise from the
    initial baseline. Returns the detected s value, or None if detection fails.
    """
    ...
    # sort + dedupe so resampling is well-defined
    order = np.argsort(s)
    s, z = s[order], z[order]
    ...
    for i in range(0, zs.size - win):
        ...
        if rise >= need_rise and zs[i + win] >= baseline + baseline_tol:
            # liftoff = first crossing of baseline+tol within the confirmed region
            j0 = max(0, i - win)
...
def detect_elevation_start_adaptive(
    ...
    **kwargs,
    ...
    """
    Try absolute thresholds first; on failure retry with thresholds scaled to
    the profile's own height range. Returns (s_start_or_None, mode_str).
    """
...
def align_profiles_to_reference(store_list, reference_substr,
                                smoothing_factor=0.0, **detect_kwargs):
    """
    Shift every profile's s axis so its detected elevation-start coincides with
    the reference profile's detected start. Rebuilds spline_z and spline_kappa
    on the shifted axis. Mutates the stores in place. Reusable across runs.
    """
    # 1) reference start
    ref_start = None
    ...
    # 2) align everything (reference self-aligns to shift ~0)
    for label, store in store_list:
        for method, d in store.items():
    ...
    # ref_start is the aligned slope-start location (usable as a left cutoff)
    return ref_start


def find_profile_peak_s(store, method_substr):
    """
    Return the (aligned) s coordinate of the maximum elevation of the profile
    whose method name contains method_substr, or None if it can't be found.
    """
    for method, d in store.items():
...
def trim_profiles_to_cutoff(store_list, s_cutoff=None, s_min=None, smoothing_factor=0.0):
    """
    Trim every profile to the window [s_min, s_cutoff] (either bound optional)
    and rebuild spline_z / spline_kappa on the trimmed arrays. Mutates the
    stores in place. Reusable across runs. Profiles with too few remaining
    points are warned about and left untrimmed.
    """
    ...
    for label, store in store_list:
        for method, d in store.items():
            ...
            if n_keep == s.size:
                continue  # nothing outside the window
            if n_keep < 4:
                print(f"WARNING [{label} – {method}]: only {n_keep} points in "
                      f"[{lo_txt}, {hi_txt}] m -> left untrimmed.")
                continue
            # Trim only arrays whose length matches s (x/y/t may differ -> skipped)
...
# ── Align all profiles to the reference elevation-start ──────────────────────
s_start_cutoff = None  # aligned slope-start; set by alignment, used as left cutoff
