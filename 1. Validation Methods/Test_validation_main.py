...
# ── Physical-measurement reference lives at the root of the results tree ───────
# The Physical_meas_*.npz files are NOT inside the deep timestamped folders, so
# they are loaded from here. Only the VALIDATION (physical) prefix is redirected
# to this root; calculation files keep loading from each results folder below.
PHYSICAL_DIR = r"D:\Validation_results"

# ── Calculation result folders (one result set / plot / CSV per folder) ───────
# Each of these deep timestamped folders is compared against the SAME physical
# reference. Add more folders here, or pass them on the command line:
#   python Test_validation_main.py <folder1> <folder2> …
RESULTS_DIRS = [
    r"D:\Validation_results\2026_05_22\10_42_10\1779439330",
    # add more calculation folders here …
]

# ── Output behaviour ──────────────────────────────────────────────────────────
SHOW_PLOTS = True    # mirror current behaviour: block on plt.show() per folder
SAVE_PLOTS = False   # when running unattended, savefig a PNG per folder instead
...
# ── npz loading ───────────────────────────────────────────────────────────────
def load_store(directory, prefixes, label):
    """Load every .npz under `directory` whose name matches one of `prefixes`.

    Returns a {method: {x, y, z, s, t, kappa, **extras}} dict, exactly as the
    original VALIDATION / CALCULATION loops produced.
    """
    files = _find_by_prefix(directory, prefixes)
    store = {}  # Access variables by method name e.g. store["AHN4 DTM"]["x"]
    try:
        for fname in files:
            fpath = os.path.join(directory, fname + ".npz")
        ...
        sys.exit(f"Error: No files found for {label}. Check filename and if does not have .npz")
    return store


# ── Process one calculation folder against the physical reference ─────────────
def process_folder(results_dir):
    print(f"\n{'=' * 80}\nProcessing folder: {results_dir}\n{'=' * 80}")

    # Reload a FRESH physical store for every folder: alignment/trim mutate the
    # store in place (shift s, cut arrays), so each folder must start from clean
    # physical data. The physical reference always comes from PHYSICAL_DIR (root).
    results_validation  = load_store(PHYSICAL_DIR, VALIDATION_PREFIXES, "VALIDATION")
    results_calculation = load_store(results_dir, CALCULATION_PREFIXES, "CALCULATION")
    ...
    for vm, vd in results_validation.items(): # Vm is validation method, vd is validation data
        for cm, cd in results_calculation.items():
            ...
            s_row  = np.linspace(s_min, s_max, 500) # Using a new s to have uniform sample intervals
            kappa_v = vd["spline_kappa"](s_row)
            kappa_c = cd["spline_kappa"](s_row)
            diff   = kappa_v - kappa_c
    ...
    ax_z.set_title(f"Elevation profile  —  {os.path.basename(results_dir.rstrip(os.sep))}")
    ax_z.legend()
    ax_z.grid(True)
    ...
    if SAVE_PLOTS:
        png_path = os.path.join(r"D:\Validation_results\Statistics", f"{t}.png")
...
def main():
    p = argparse.ArgumentParser(
        description="Compare each calculation folder against the physical "
                    "reference loaded from the results root."
    )
    p.add_argument("folders", nargs="*", metavar="FOLDER",
                   help="calculation result folder(s); overrides RESULTS_DIRS")
    args = p.parse_args()

    results_dirs = args.folders if args.folders else RESULTS_DIRS
